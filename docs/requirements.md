# Dokumen Persyaratan (Requirements Document) — Konsolidasi

> Versi gabungan: Persyaratan 1-18 (inti, dengan Persyaratan 6 versi peta + radius) dan Persyaratan 19-23 (fitur tambahan). Sumber tunggal yang menggantikan berkas terpisah di `specs/`.
...
- **Peta_Interaktif**: Komponen peta pada halaman Scraper tempat Pengguna menentukan titik pusat pencarian secara visual (klik, geser pin, atau cari nama tempat)
- **Titik_Pusat**: Koordinat (latitude, longitude) pusat area pencarian yang ditentukan Pengguna pada Peta_Interaktif
- **Radius_Pencarian**: Jarak (dalam kilometer) dari Titik_Pusat yang membatasi area pengambilan dan penyaringan Lead
...
1. WHEN Pengguna memasukkan kata kunci pencarian dan menentukan lokasi (baik berupa teks lokasi maupun Titik_Pusat dari Peta_Interaktif), THE Scraper SHALL mengambil maksimal 500 data bisnis dari Google Maps yang sesuai dengan kata kunci dan lokasi yang dimasukkan
...
9. WHEN Pengguna membuka halaman Scraper, THE Platform SHALL menampilkan Peta_Interaktif di mana Pengguna dapat menentukan Titik_Pusat pencarian dengan tiga cara: mengklik lokasi pada peta, menggeser (drag) pin penanda, atau mengetik nama tempat pada kotak pencarian peta; dan THE Platform SHALL menampilkan label lokasi hasil reverse-geocoding dari Titik_Pusat tersebut
10. WHEN Pengguna mengatur Radius_Pencarian melalui slider dalam rentang 1 km hingga 20 km, THE Peta_Interaktif SHALL menampilkan lingkaran radius yang berpusat pada Titik_Pusat dan memperbaruinya secara langsung (kurang dari 200 milidetik) mengikuti nilai radius tanpa memuat ulang halaman
11. IF Pengguna memulai scraping tanpa menentukan Titik_Pusat pada peta saat mode peta aktif, THEN THE Platform SHALL menolak eksekusi dan menampilkan pesan yang meminta Pengguna menaruh titik pusat terlebih dahulu
12. WHEN Pengguna menjalankan scraping berbasis Titik_Pusat dan Radius_Pencarian, THE Scraper SHALL menentukan sejumlah titik pencarian (grid) di dalam radius secara proporsional terhadap besar radius (satu titik untuk radius kecil, grid bertambah untuk radius lebih besar), menjalankan pencarian pada setiap titik, lalu menggabungkan hasilnya dengan deduplikasi
13. WHEN hasil scraping terkumpul, THE Scraper SHALL menyaring hasil sehingga hanya bisnis yang berada pada jarak (dihitung dengan formula haversine dari Titik_Pusat) kurang dari atau sama dengan Radius_Pencarian yang disimpan ke daftar Lead
14. THE Scraper SHALL menyimpan koordinat (latitude dan longitude) untuk setiap Lead bila tersedia, serta website dan email bila tersedia, untuk mendukung penyaringan jarak dan pengayaan data
