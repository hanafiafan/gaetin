...
echo ============================================================
echo  Hellens - skrip jalan lokal (dev)
echo  Prasyarat: Docker Desktop berjalan + Node.js 20+ terpasang
echo ============================================================

echo.
echo [1/5] Menyalakan PostgreSQL (Docker)...
docker start hellens-db 1>nul 2>nul || docker run -d --name hellens-db -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=password -e POSTGRES_DB=hellens_scraper -p 5434:5432 postgres:16-alpine

echo [2/5] Menyalakan Redis (Docker)...
docker start hellens-redis 1>nul 2>nul || docker run -d --name hellens-redis -p 6379:6379 redis:7-alpine
