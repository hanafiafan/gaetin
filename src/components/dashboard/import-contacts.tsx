...
export default function ImportContacts() {
  ...
  const [mapping, setMapping] = useState<Record<Field, number>>({
    phone: -1, name: -1, label: -1, email: -1, city: -1, category: -1,
  });
  ...
  async function runImport() {
    ...
    const rows = records
      .map((r) => {
        ...
        return { phone: get("phone") ?? "", name: get("name"), label: get("label"), email: get("email"), city: get("city"), category: get("category") };
      })
      .filter((r) => r.phone);
    ...
    setImporting(false);
    if (!res.ok) { setError(j?.error?.message ?? "Gagal mengimpor"); return; }
    setSummary(j.data);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {summary && (
        <div className="cg-card cg-sheet rounded-xl p-5 space-y-3">
