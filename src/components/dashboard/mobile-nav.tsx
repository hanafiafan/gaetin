          ...
          <div className="cg-sidebar absolute left-0 top-0 flex h-full w-[300px] max-w-[88vw] flex-col overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-foreground/15 px-4 py-4">
              ...
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-lg text-foreground/70 hover:bg-foreground/10 hover:text-foreground"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
                    ...
                    <div className="space-y-0.5">
                      {items.map((item) => {
                        ...
                        if (locked) {
                          return (
                            <button
                              ...
                              className="flex w-full items-start gap-3 rounded-lg px-3 py-3 text-left font-semibold text-foreground/60 transition-colors duration-200 hover:bg-foreground/10"
                            >
                              <Icon className="mt-0.5 h-5 w-5 shrink-0 opacity-40" />
                              <span className="min-w-0 flex-1 opacity-60">
                                <span className="block">{item.label}</span>
                                <span className="mt-0.5 block text-sm font-normal leading-snug">{item.desc}</span>
                              </span>
                              <span className="mt-0.5 shrink-0 rounded bg-warning/15 px-1.5 py-0.5 text-xs font-semibold uppercase text-warning">Bisnis</span>
                            </button>
                          );
                        }
                        ...
                        return (
                          ...
                          >
                            <Icon className={cn("mt-0.5 h-5 w-5 shrink-0", active && TONE_TEXT[group.tone])} />
                            <span className="min-w-0">
                              <span className="block">{item.label}</span>
                              {/* Di item aktif latarnya lime, dan
                                  text-muted-foreground di atas lime praktis
                                  tidak terbaca. */}
                              <span
                                className={cn(
                                  "mt-0.5 block text-sm font-normal leading-snug",
            ...
            <div className="shrink-0 border-t border-foreground/15 p-3">
              {isSuperAdmin && (
                <Link
                  href="/admin"
                  onClick={close}
                  className="mb-1 flex items-center gap-3 rounded-lg px-3 py-3 font-semibold text-foreground transition hover:bg-foreground/10"
                >
