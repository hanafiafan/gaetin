import { NextRequest, NextResponse } from "next/server";
import type { Prisma } from "@prisma/client";
import { prisma } from "@/lib/db/prisma";
import { getSession } from "@/lib/auth/session";
import { CreateEmailBlastSchema } from "@/lib/validators/email-blast";
import { fail } from "@/lib/api";

export async function GET() {
  const session = await getSession();
  if (!session) return fail("AUTH_003", "Tidak terautentikasi", 401);

  const items = await prisma.emailBlast.findMany({
    where: { workspaceId: session.workspace.id },
    orderBy: { createdAt: "desc" },
    select: {
      id: true,
      name: true,
      subject: true,
      status: true,
      totalRecipients: true,
      sentCount: true,
      failedCount: true,
      createdAt: true,
    },
  });
  return NextResponse.json({ success: true, data: items });
}

export async function POST(req: NextRequest) {
  const session = await getSession();
  if (!session) return fail("AUTH_003", "Tidak terautentikasi", 401);
  const workspaceId = session.workspace.id;

  let body: unknown;
  try {
    body = await req.json();
  } catch {
    return fail("VAL_001", "Body permintaan tidak valid", 400);
  }
  const parsed = CreateEmailBlastSchema.safeParse(body);
  if (!parsed.success) return fail("VAL_001", "Validasi gagal", 400, parsed.error.flatten().fieldErrors);

  const { name, subject, bodyText, label } = parsed.data;

  const where: Prisma.ContactWhereInput = { workspaceId, email: { not: null } };
  if (label) where.label = label;

  const contacts = await prisma.contact.findMany({ where, select: { id: true }, take: 10000 });
  if (contacts.length === 0) return fail("EMPTY", "Tidak ada kontak dengan email yang cocok", 400);

  const blast = await prisma.emailBlast.create({
    data: {
      workspaceId,
      name,
      subject,
      bodyText,
      variables: { label: label ?? null },
      totalRecipients: contacts.length,
      status: "DRAFT",
      createdById: session.user.id,
    },
  });

  await prisma.emailBlastMessage.createMany({
    data: contacts.map((c) => ({ emailBlastId: blast.id, contactId: c.id })),
  });

  return NextResponse.json(
    { success: true, data: { id: blast.id, totalRecipients: contacts.length } },
    { status: 201 },
  );
}
